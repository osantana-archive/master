...
* │  - SYS_DIR - Diretório dos Arquivos                                       │
* ├───────────────────────────────────────────────────────────────────────────┤
...
PARAMETERS CT
ARQ = SYS_DIR+TRIM(UTL_DBF[DIM(CT,1,5)])
...
IF FILE(ARQ) .AND. ACT
   ...
   FOR CP = 1 TO 15
       MCR = STRZERO(CP,2)
       IND = SYS_DIR+TRIM(LEFT(UTL_DBF[DIM(CT,1,5)],6)) + MCR + ".NTX"
       IF FILE(IND)
          ID&MCR = IND
       ELSE
          ID&MCR = ""
...
* FUNCAO: MAKETEMP                                                            *
*-----------------------------------------------------------------------------*
* UTILIZACAO: CRIA ARQUIVOS TEMPORARIOS DE DADOS.                             *
* SINTAXE   : MAKETEMP (matriz com os "campos,tipos,tamanhos do arquivo,etc.",*
*                       prefixo do nome do arquivo, chave do indice,          *
*                       nome da UDF para preenchimento do arquivo temporario) *
*******************************************************************************

FUNCTION MAKETEMP
PRIVATE MT_P1,MT_P2,MT_P3,MT_P4,MT_T1,MT_T2
PARAMETERS MT_P1,MT_P2,MT_P3,MT_P4
MESSAGE("Aguarde...Criando Arquivo Temporário Auxiliar...")
MT_T2 = ALIAS()
SELECT 0
IF TYPE("UTL_ARQ")<>"A"
   PUBLIC UTL_ARQ[1],UTL_IARQ
   UTL_IARQ = 0
ENDIF
UTL_IARQ = UTL_IARQ + 1
UTL_ARQ[UTL_IARQ] = MT_P2+STRZERO(UTL_IARQ,3)
FOR MT_T1 = 1 TO LEN(MT_P1) STEP 4
    DATABASE(MT_P1[MT_T1],MT_P1[MT_T1+1],MT_P1[MT_T1+2],MT_P1[MT_T1+3])
NEXT
DATABASE(SYS_DIR+UTL_ARQ[UTL_IARQ]+"D.$$$")
USE(SYS_DIR+UTL_ARQ[UTL_IARQ]+"D.$$$")
IF TYPE("MT_P3") = "C"
   INDEX ON &MT_P3 TO (SYS_DIR+UTL_ARQ[UTL_IARQ]+"I.$$$")
ENDIF
MT_T1 = ALIAS()
MT_P4 = MT_P4 + "(MT_T1)"
SELECT &MT_T2
...
* FUNCAO: ZAPTEMP                                                             *
*-----------------------------------------------------------------------------*
* UTILIZACAO: APAGA O ULTIMO ARQUIVO TEMPORARIO CRIADO.                       *
* SINTAXE   : ZAPTEMP ()                                                      *
*******************************************************************************
...
IF TYPE("UTL_ARQ") <> "A"
   RETURN(.F.)
ENDIF
ZT_T1 =UTL_ARQ[LEN(UTL_ARQ)]+"D"
SELECT &ZT_T1
USE
ERASE (SYS_DIR+ZT_T1+".$$$")
ERASE (SYS_DIR+LEFT(ZT_T1,7)+"I.$$$")
UTL_IARQ = UTL_IARQ - 1
RETURN(.T.)
...
* FUNCAO: LOOKHERE                                                            *
*-----------------------------------------------------------------------------*
* UTILIZACAO: MOSTRA UMA MENSAGEM COM EFEITO INTERMITENTE LIMITADO            *
* SINTAXE   : LOOKHERE (Titulo,Tempo de Intermitencia)                        *
*******************************************************************************

FUNCTION LOOKHERE
PRIVATE LH_P1,LH_P2
LH_P2 = 1
PARAMETERS LH_P1,LH_P2
MESSAGE(LH_P1,.F.,.T.)
DELAY(LH_P2,.T.)
MESSAGE(LH_P1)
RETURN(.T.)
...
* FUNCAO: DSP                                                                 *
*-----------------------------------------------------------------------------*
* UTILIZACAO: MOSTRA OS DADOS NA COORDENADA ESPECIFICADA COM COR DESTAQUE     *
...
* SINTAXE   : PRT (Linha,Coluna,Mensagem,Tamanho Maximo)                      *
*******************************************************************************
...
* FUNCAO: CHKDATE                                                             *
*-----------------------------------------------------------------------------*
* UTILIZACAO: VERIFICA A INTEGRIDADE DE 2 DATAS EM 2 GET'S CONSECUTIVOS       *
* SINTAXE   : CHKDATE (@ Data Inicial, @ Data Final, No Campo a pesquisar no  *
*                     bco de dados)                                           *
*******************************************************************************

FUNCTION CHKDATE
PRIVATE CD_P1,CD_P2,CD_P3,CD_P4,CD_P5,CD_T1
PARAMETERS CD_P1,CD_P2,CD_P3,CD_P4,CD_P5
IF CD_P2 < CD_P1 .AND. .NOT. (LASTKEY() = 27 .OR. LASTKEY() = 5 .OR. EMPTY(CD_P2))
   WARNING("A Data Final näo pode ser menor que a Data Inicial...Redigite.")
   RETURN(.F.)
ENDIF
CD_T1 = FIELD(CD_P3)
SET SOFTSEEK ON
...
IF .NOT. FOUND()
   IF .NOT. EMPTY(&CD_T1)
      CD_P1 = &CD_T1
   ELSE
      WARNING("Data Inicial näo é válida. Redigite.")
      RETURN(LASTKEY() = 5)
   ENDIF
ENDIF
SEEK CD_P2
IF .NOT. FOUND()
   IF EMPTY(CD_P2) .OR. EMPTY(&CD_T1)
      GOTO BOTTOM
   ENDIF
   CD_P2 = &CD_T1   
   IF EMPTY(&CD_T1)
      WARNING("Data Final näo é válida. Redigite.")
      RETURN(LASTKEY() = 5)
   ENDIF
ENDIF         
SET SOFTSEEK OFF
IF TYPE("CD_P4")="D"
   IF CD_P1 < CD_P4
      LOOKHERE("Data Inicial está fora da Faixa Especificada. Redigite.")
      RETURN(LASTKEY() = 5)
   ENDIF
ENDIF
IF TYPE("CD_P5")="D"
   IF CD_P2 > CD_P5
      LOOKHERE("Data Final está fora da Faixa Especificada. Redigite.")
      RETURN(LASTKEY() = 5)
   ENDIF
ENDIF
RETURN(.T.)
...
* FUNCAO: CHKMONTH                                                            *
*-----------------------------------------------------------------------------*
* UTILIZACAO: VERIFICA A INTEGRIDADE DE 2 DATAS (MM/YY) EM 2 GET'S CONSECTIV. *
* SINTAXE   : CHKDATE (@ Data Inicial, @ Data Final, No Campo a pesquisar no  *
*                     bco de dados)                                           *
*******************************************************************************

FUNCTION CHKMONTH
PRIVATE CM_P1,CM_P2,CM_P3,CM_P4,CM_P5,CM_T1
PARAMETERS CM_P1,CM_P2,CM_P3,CM_P4,CM_P5
IF .NOT. VDATE(CM_P1)
   WARNING("A Data Inicial é Inválida. Redigite.")
   RETURN(LASTKEY() = 5)
ENDIF
IF .NOT. VDATE(CM_P2)
   WARNING("A Data Final é Inválida. Redigite.")
   RETURN(LASTKEY() = 5)
ENDIF   
IF CTOD("01/"+CM_P2) < CTOD("01/"+CM_P1) .AND. .NOT. (LASTKEY() = 27 .OR. LASTKEY() = 5 .OR. EMPTY(CM_P2))
   WARNING("A Data Final näo pode ser menor que a Data Inicial...Redigite.")
   RETURN(.F.)
ENDIF
CM_T1 = FIELD(CM_P3)
...
SEEK CTOD("01/"+CM_P2)
IF .NOT. FOUND()
   IF EMPTY(CM_P2) .OR. EMPTY(&CM_T1)
      GOTO BOTTOM
   ENDIF
   CM_P2 = RIGHT(DTOC(&CM_T1),5)   
   IF EMPTY(&CM_T1)
      WARNING("Data Final näo é válida. Redigite.",1.5)
      RETURN(LASTKEY() = 5)
   ENDIF
ENDIF         
SET SOFTSEEK OFF
IF TYPE("CM_P4")="C"
   IF CTOD("01/"+CM_P1) < CTOD("01/"+CM_P4)
      WARNING("Data Inicial está fora da Faixa Especificada. Redigite.") 
      RETURN(LASTKEY() = 5)
   ENDIF
ENDIF
IF TYPE("CM_P5")="C"
   IF CTOD("01/"+CM_P2) > CTOD("01/"+CM_P5)
      WARNING("Data Final está fora da Faixa Especificada. Redigite.")
      RETURN(LASTKEY() = 5)
   ENDIF
ENDIF
RETURN(.T.)
